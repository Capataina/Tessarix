...
- **Navigation** — `src/components/nav/GraphNav.tsx` is the front door (replacing the old `Catalog` card grid): a category picker → a layered prerequisite-DAG **tree diagram** (`src/components/nav/LessonTree.tsx`) that lays the category's lessons out as connected nodes with curved SVG prerequisite edges (roots on top, dependents fan down), recoloured to the category palette.
- **Concept graph** — `src/lib/graph/` (`meta.ts` authored per-lesson metadata; `linker.ts` the pure concept index + deterministic cross-page linker for LLM-generated prose; `build.ts` the registry-coupled graph builder; `themes.ts` the per-category palettes). It now **also links authored lesson prose**: `normalize.ts` (a `wink-lemmatizer` lemma matcher), `targets.ts` (per-concept `strong` cross-domain / `contextual` in-domain scope), `headers.generated.ts` (H2/H3 section anchors harvested by `scripts/harvest-headers.mjs`), `match.ts` (the scoped matcher), and `linkify.tsx` (the MDX `p`/`li` overrides that render `<ConceptLink>`, whose hover surfaces an instant summary). Generation is separated from linking throughout.
...
- **LLM layer** — `src/lib/llm/` (Ollama-backed; powers the chatbot, catalog recommender, state-aware widget explanations via `<WidgetExplainer>`, tiered hints, and the widget mini-lesson via `miniLesson.ts` — streamed token-by-token, then markdown-rendered + concept-linked through `format.ts`). `client.ts`/`hooks.ts` fall back to a direct Ollama `fetch` when not inside Tauri, so LLM features work in the dev/preview/Playwright browser too. All calls are serialised through a client-side **priority queue** (`queue.ts`; reader-initiated "high" jumps ahead of background auto-generation "low") so the single-slot local model isn't starved; widget state-captions are **visibility-gated** (a widget generates only once it scrolls into view). Two on-demand surfaces added: **hover instant-summaries** (`contextual.ts` + `<ConceptLink>`) and **"explain here"** (`explain.ts` + `src/components/ExplainHere.tsx` — select → right-click → a drawer streams a grounded explanation + follow-up chat).
...
| **concept-graph + navigation** | [`plans/curriculum-graph.md`](plans/curriculum-graph.md) (design); `src/lib/graph/`, `src/components/nav/GraphNav.tsx` + `LessonTree.tsx` | The typed concept graph derived from lesson metadata + the deterministic linker, and the graph-nav front door (a layered-DAG tree diagram) that renders it. No dedicated `systems/*.md` yet (coverage gap). | working (new 2026-06-30) | unstable |
