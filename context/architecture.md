...
- **LLM layer** — `src/lib/llm/` (Ollama-backed; powers the chatbot, catalog recommender, state-aware widget explanations via `<WidgetExplainer>`, tiered hints, and the widget mini-lesson via `miniLesson.ts` — streamed token-by-token, then markdown-rendered + concept-linked through `format.ts`). `client.ts`/`hooks.ts` now fall back to a direct Ollama `fetch` when not inside Tauri, so LLM features work in the dev/preview/Playwright browser too.
