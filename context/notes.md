...
- [visual-identity](notes/visual-identity.md) — "Tailored leather notebook meets a configured terminal": warm chocolate-luxe surfaces (COS / Ralph Lauren register) + terminal-grade mono structure, replacing the old neon dashboard. Signature: the tesseract mark + terminal-pane widgets + boot-cascade motion. Design values flow from the [styling system](systems/styling-system.md); never hardcode a colour.
